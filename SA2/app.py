...
def miningPool():
    ...
    if request.method == "POST":
        ...
        # Create status variable and set it to False
        status = False
        # get status and minedBlock as return values after calling the minePendingTransactions() method
        status, minedBlock = chain.minePendingTransactions(minerAddress)
        
        # Check if The status is "Mined" and then run the for loop
        if(status=="Mined"):
