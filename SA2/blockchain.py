...
class BlockChain():
    ...
    def minePendingTransactions(self, minerAddress):
        # Check if peerBlocks has some entries on it i.r length is greater then 0
        if (len(self.peerBlocks) > 0):
            # Return Status as "PeerBlocksPending" and None
        ...
        for miner in self.miners:
            if miner.address == minerAddress:
                ...
                if(isBlockAdded):  
                    ...
                    # Return status as "Mined" and currentBlock
                    return("Mined ",currentBlock)
        # Return "NotMined" and None
